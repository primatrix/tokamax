...
class KimiDeltaAttentionTest(parameterized.TestCase):
  ...
  def test_xla_chunked_matches_xla(self, dtype):
    q, k, v, g, beta, initial_state = _make_inputs(dtype)
    q = jnp.concatenate([q, q[:, :, :2]], axis=2)
    k = jnp.concatenate([k, k[:, :, :2]], axis=2)
    v = jnp.concatenate([v, v[:, :, :2]], axis=2)
    g = jnp.concatenate([g, g[:, :, :2]], axis=2)
    beta = jnp.concatenate([beta, beta[:, :, :2]], axis=2)
  ...
  def test_xla_chunked_gradients_match_xla(self):
    q, k, v, g, beta, _ = _make_inputs(jnp.float32)
    q = (jnp.tile(q, (1, 1, 3, 1))[:, :, :16]) * 0.25
    k = (jnp.tile(k, (1, 1, 3, 1))[:, :, :16]) * 0.25
    v = (jnp.tile(v, (1, 1, 3, 1))[:, :, :16]) * 0.25
    g = (jnp.tile(g, (1, 1, 3, 1))[:, :, :16]) * 0.25
    beta = (jnp.tile(beta, (1, 1, 3))[:, :, :16]) * 0.25
  ...
  def test_varlen_gate_l2norm_matches_reference(self):
    ...
    chunked_output, chunked_state = api.kimi_delta_attention(
        q,
        k,
        v,
        g,
    ...
    chex.assert_trees_all_close(chunked_output, output, atol=0.01, rtol=0.01)
    chex.assert_trees_all_close(chunked_state, final_state, atol=0.01, rtol=0.01)
