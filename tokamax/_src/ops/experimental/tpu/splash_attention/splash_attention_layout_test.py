...
@pytest.mark.parametrize("mixed_prefix", [False, True])
def test_native_prefix_full_metadata(mixed_q, mixed_prefix):
  arrays, _ = _inputs(512, 512, 72, 72)
  q_ids = np.repeat(np.array([-3, 0, 7, -3], np.int32), 128)
  kv_ids = np.repeat(np.array([-3, 0, 0, 7], np.int32), 128)
  if mixed_q:
