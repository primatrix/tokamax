...
def _flash_attention_dkv_kernel(
  ...
  split_query_compute = (
      ...
      and dq_scratch_ref is not None and config.bwd_scale_after_dot
      and not config.bwd_dq_first and not config.bwd_dv_last
  )

  def split_query_body(i, _, has_partial_mask=False):
    # Keep the DMA windows, compute-KV size and output rounding boundaries.
    # Q2048 bounds the RHS transpose while dK/dV are produced as [D, KV].
    # dQ's KV order and the two-half dK/dV reduction grouping are unchanged.
    q_compute = 2048
    k_slice = pl.ds(i * bkv_compute, bkv_compute)
    keys, values = k_ref[:, k_slice], v_ref[:, k_slice]
    dk_init = jnp.zeros((keys.shape[0], bkv_compute), jnp.float32)
    dv_init = jnp.zeros((values.shape[0], bkv_compute), jnp.float32)
    ...
    def query_half(qi, carry):
      ...
      dv = lax.dot_general(
          dout, p.astype(dout.dtype), NT_DIM_NUMBERS,
          preferred_element_type=jnp.float32,
      )
      dp = lax.dot_general(
          values, dout, TN_DIM_NUMBERS, preferred_element_type=jnp.float32,
      )
      ds = ((dp - di_ref[:1, q_slice]) * p).astype(query.dtype)
      dk = lax.dot_general(
          query, ds, NT_DIM_NUMBERS, preferred_element_type=jnp.float32,
      )
      dq = lax.dot_general(
  ...
  def run_inner_loop(has_partial_mask):
    if split_query_compute:
      lax.fori_loop(
          0, num_iters, partial(split_query_body, has_partial_mask=has_partial_mask),
