...
def _flash_attention_dkv_kernel(
  ...
  def body(i, _, has_partial_mask=False):
    ...
    def compute_packed_dkv():
      # The two useful diagonal products share the same Q reduction order.
      # On a 256-column MXU the padded N=128 halves fit one physical tile;
      # the unwanted off-diagonal products replace otherwise padded lanes.
...
def _splash_attention_bwd_dkv(
  ...
  dynamic_grid = mask_info.active_rows is not None
  pack_dkv = (
      native_layout and head_dim_qk <= 128 and head_dim_v <= 128
